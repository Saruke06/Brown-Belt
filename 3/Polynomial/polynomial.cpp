...
public:
  class IndexProxy {
  public:
	  IndexProxy(Polynomial& poly_, size_t degree_)
	  : poly(poly_), degree(degree_)
	  {}
	  ...
	  IndexProxy& operator=(const T& value) {
  	  ...
  	  friend bool operator==(const IndexProxy& lhs, const T& rhs) {
  		  return T(lhs)==rhs;
  	  }

  	  friend ostream& operator<< (ostream& os, const IndexProxy& ip) {
  		  return os << T(ip);
  	  }
